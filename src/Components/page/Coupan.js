...
const Coupan = () => {
  ...
  let getcoupon_list = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/fund/getAll`, options)
      .then((res) => {
  ...
  const exportToExcel = () => {
    ...
    saveAs(data, 'memberfund_list.xlsx');
  };

  const downloadPDF = () => {
    const doc = new jsPDF();
    const tableColumn = ["Sr No.", "MemberId", "Name", "Narration", "Amount", "Payment Status", "Current Balance"];
    const tableRows = [];
    ...
    doc.text("Member Fund List", 14, 15);
    doc.save("memberfund_list.pdf");
  };
  ...
  const renderSubcategoryData = (data, index) => {
    ...
    return (
      <tr key={index}>
        ...
        >
          <div class="line--limit-2 max-w-200" style={{scrollbarWidth:'thin', overflowX:'hidden', height:'50px', textAlign: "justify" }}>
            {data?.narration}
          </div>
        </td>
        ...
        <div className="col-lg-9 col-md-8" style={{ marginTop: "60px" }}>
          <div className="mt-3 mb-5">
            <h2 className="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
              ...
              MEMBER FUND REPORT<span className="badge badge-soft-dark radius-50">{count}</span>
            </h2>
          </div>
